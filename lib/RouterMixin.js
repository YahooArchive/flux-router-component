...
RouterMixin = {
    componentDidMount: function() {
        ...
        if (self._enableScroll) {
            self._scrollListener = lodash.debounce(function saveScrollPosition(e) {
                var historyState = (self._history.getState && self._history.getState()) || {};
                historyState.scroll = {x: window.scrollX, y: window.scrollY};
                debug('remember scroll position', historyState.scroll);
                self._history.replaceState(historyState);
            }, 200);
