...
NavLink = React.createClass({
    ...
    render: function() {
        var context = this.props.context,
            routerName = this.props.routeName || this.props.name; // 'name' property is deprecated
        if (!this.props.href && routerName && context && context.makePath) {
            this.props.href = context.makePath(routerName, this.props.navParams);
        }
